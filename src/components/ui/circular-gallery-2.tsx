...
interface CircularGalleryProps
  ...
  /**
   * Enable auto-scroll animation for infinite loop.
   * @default false
   */
  autoScroll?: boolean;
  /**
   * Speed of auto-scroll animation.
   * @default 0.5
   */
...
class Title {
  ...
  createMesh() {
    ...
    const textHeight = this.plane.scale.y * 0.18; // Smaller - 18% of image height
    const textWidth = textHeight * aspect;
    this.mesh.scale.set(textWidth, textHeight, 1);
    // Position just below the image
    this.mesh.position.y = -this.plane.scale.y * 0.5 - textHeight * 0.7;
    this.mesh.position.z = 0.1; // Slightly in front to avoid z-fighting
...
const CircularGallery = ({
  ...
  }, [items, bend, borderRadius, scrollSpeed, scrollEase, autoScroll, autoScrollSpeed, fontClassName]);
  ...
  return (
    <div
      ...
      className={cn(
        ...
        "text-foreground font-extrabold text-[120px]",
        fontClassName,
        className,
      )}
      {...props}
